    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="w-full border-b border-arcon-gray-border bg-white/80 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="text-arcon-gray-primary font-roboto">{fullName}</span>
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="default" className="bg-arcon-blue text-white hover:bg-arcon-blue/90">Edit</Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                ...
                <form onSubmit={handleSaveProfile} className="space-y-4 mt-2">
                  ...
                  {saveError && <p className="text-red-600 text-sm font-roboto">{saveError}</p>}
                  <DialogFooter>
                    <Button type="button" variant="secondary" onClick={() => setIsDialogOpen(false)}>Cancel</Button>
                    <Button type="submit" disabled={saving} className="bg-arcon-blue text-white hover:bg-arcon-blue/90">{saving ? "Saving..." : "Save"}</Button>
                  </DialogFooter>
                </form>
              </DialogContent>
            </Dialog>
            <Button variant="ghost" onClick={handleLogout} className="text-arcon-blue">Logout</Button>
          </div>
        </div>
      </header>
